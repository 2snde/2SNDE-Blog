...
const T = {
  FR: {
    ...
    heroMainSub: "Sales Strategist & Builder",
  },
  EN: {
...
export default function App(){
  ...
  const togglePinPost=(id,e)=>{if(e)e.stopPropagation();setArticles(p=>p.map(a=>({...a,pinned:a.id===id?!a.pinned:false})));};
  ...
  const pinnedCreation=creations.find(c=>c.pinned);
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav{position:sticky;top:0;z-index:200;background:rgba(7,9,15,0.88);backdrop-filter:blur(24px);border-bottom:1px solid transparent;transition:all 0.4s}.nav.stuck{border-bottom-color:var(--border);background:rgba(7,9,15,0.96)}.nav-inner{max-width:1100px;margin:0 auto;height:64px;padding:0 2rem;display:flex;align-items:center;justify-content:space-between;gap:0.75rem}
        .logo{font-family:var(--fd);font-size:1.1rem;font-weight:800;color:var(--text);cursor:pointer;display:flex;align-items:center;gap:10px;transition:all 0.2s;flex-shrink:0;background:none;border:none}.logo:hover{opacity:0.75}.logo-mark{width:32px;height:32px;background:linear-gradient(135deg,var(--accent),#667eea);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:0.62rem;font-weight:800;color:#fff;box-shadow:0 0 20px var(--accent-glow);flex-shrink:0}
        ...
        /* ── LANDING ─────────────────────────────────────────────── */
        .landing{max-width:760px;margin:0 auto;padding:0 2rem}
        .landing-hero{padding:5rem 0 3rem;text-align:center}
        .landing-badge{font-family:var(--fd);font-size:0.68rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:var(--accent);margin-bottom:1.25rem}
        .landing-name{font-family:var(--fd);font-size:clamp(2.2rem,6vw,3.5rem);font-weight:800;letter-spacing:-0.04em;line-height:1.08;margin-bottom:0.5rem}
        .landing-role{font-family:var(--fd);font-size:clamp(0.92rem,2vw,1.1rem);color:var(--text2);font-weight:500}
        .landing-divider{width:40px;height:1px;background:var(--border2);margin:3rem auto}
        ...
        .landing-pin-card{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.35rem;cursor:pointer;transition:all 0.25s;position:relative;overflow:hidden}.landing-pin-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--accent),#667eea,transparent)}.landing-pin-card:hover{border-color:rgba(79,156,249,0.2);transform:translateY(-2px);box-shadow:0 12px 40px rgba(0,0,0,0.35)}
      ...
      {section==="home"&&view==="list"&&(<div className="landing">
        ...
        <Reveal delay={0.05}><div className="landing-section">
          ...
          {pinnedArticle ? (
            <div className="landing-pin-card" onClick={()=>{setSection("posts");setCurrent(pinnedArticle);setView("article");}}>
              <div className="landing-pin-label">📌 {t.pinned}</div>
              <div style={{display:"flex",gap:"0.3rem",marginBottom:"0.5rem",flexWrap:"wrap"}}>{pinnedArticle.tags.map((x,i)=><span key={x} className="chip" style={cs(i)}>{x}</span>)}<LangChip lang={pinnedArticle.lang}/></div>
              <div className="landing-pin-title">{pinnedArticle.title}</div>
              <div className="landing-pin-excerpt">{exc(pinnedArticle.content)}…</div>
              <div className="landing-pin-meta"><span>{fDate(pinnedArticle.date,lang)}</span><span>·</span><span>{rt(pinnedArticle.content)} {t.readMin}</span></div>
            </div>
          ) : <div className="landing-empty">{lang==="FR"?"Épinglez un post pour le mettre en avant ici.":"Pin a post to feature it here."}</div>}
        </div></Reveal>

        {/* Pinned Creation */}
        <Reveal delay={0.1}><div className="landing-section">
          <div className="landing-section-header">
            <div className="landing-section-title">{t.creations}</div>
            <button className="landing-see-all" onClick={()=>navTo("creations")}>{t.seeAll}</button>
          </div>
          {pinnedCreation ? (()=>{const st=SS[pinnedCreation.status]||SS.wip;return(
            <div className="landing-pin-card" onClick={()=>navTo("creations")}>
              <div className="landing-pin-label">📌 {t.pinned}</div>
              <div style={{display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"0.4rem"}}><span className="landing-pin-title" style={{marginBottom:0}}>{pinnedCreation.title}</span><LangChip lang={pinnedCreation.lang}/><span className="creation-status" style={{background:st.bg,color:st.c,border:`1px solid ${st.bd}`}}>{t[pinnedCreation.status]}</span></div>
              <div className="landing-pin-excerpt">{pinnedCreation.description}</div>
              <div className="creation-tech" style={{marginTop:"0.6rem"}}>{(pinnedCreation.tech||[]).map(x=><span key={x} className="tech-tag">{x}</span>)}</div>
            </div>);})()
          : <div className="landing-empty">{lang==="FR"?"Épinglez une création pour la mettre en avant ici.":"Pin a creation to feature it here."}</div>}
        </div></Reveal>

        {/* Pinned Talk */}
        <Reveal delay={0.15}><div className="landing-section">
          <div className="landing-section-header">
