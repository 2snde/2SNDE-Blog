...
const T = {
  FR: {
    posts: "Posts", creations: "Creations", talks: "Talks",
...
const DEF_ARTICLES = [
  ...
  { id: 2, pinned: false, lang: "FR", title: "Comment utiliser l'IA pour doubler sa productivité en sales", date: "2026-03-05", tags: ["AI", "Productivity"], images: [],
...
const DEF_CREATIONS = [
  { id: 1, lang: "EN", category: "Prototype", title: "Sales Operating System", description: "Structured public knowledge base organizing my sales methodology across three tracks.", tech: ["GitHub", "Markdown", "Open Source"], status: "live", links: [{ label: "GitHub", url: "#" }], media: [] },
  { id: 2, lang: "FR", category: "Tool", title: "Sales Offer & Quota Math Analyzer", description: "Workbook Excel couvrant le funnel math, modélisation du cycle, structures draw/bonus, avec références fiscales par pays.", tech: ["Excel", "VBA", "Financial Modeling"], status: "live", links: [], media: [] },
  { id: 3, lang: "EN", category: "Prototype", title: "Framework Builder Agent", description: "React app that extracts reusable frameworks and templates from pasted content, with AI-driven dynamic selection.", tech: ["React", "Claude API", "NLP"], status: "live", links: [{ label: "Demo", url: "#" }], media: [] },
  { id: 4, lang: "EN", category: "App", title: "Voice-First Sales Coach", description: "Voice-based coaching app with five-dimension scoring and animated SVG feedback.", tech: ["React", "Web Speech API", "SVG"], status: "live", links: [], media: [] },
  { id: 5, lang: "FR", category: "Tool", title: "Quota Reverse-Engineering Tool", description: "Outil standalone avec étapes de funnel ajustables pour reverse-engineer les quotas.", tech: ["React", "Data Viz"], status: "wip", links: [], media: [] },
];
const DEF_TALKS = [
  { id: 1, lang: "FR", type: "podcast", title: "Cold Calling en 2026 : ce qui marche encore", show: "Sales Unplugged", date: "2026-03-15", duration: "42 min", description: "Retour sur les techniques de cold call qui fonctionnent en B2B tech.", audioUrl: "", media: [] },
  { id: 2, lang: "FR", type: "call", title: "Live Discovery Call — SaaS Cybersecurity", show: "Enregistrement terrain", date: "2026-02-20", duration: "28 min", description: "Un vrai discovery call enregistré (anonymisé) avec analyse post-call.", audioUrl: "", media: [] },
  { id: 3, lang: "EN", type: "vlog", title: "My morning prospecting routine", show: "Vlog", date: "2026-01-10", duration: "12 min", description: "Video walkthrough of my routine before 10am: research, sequences, calls.", audioUrl: "", media: [] },
];
...
const CC=[["rgba(79,156,249,0.12)","#82baff","rgba(79,156,249,0.28)"],["rgba(52,211,153,0.1)","#34d399","rgba(52,211,153,0.28)"],["rgba(251,146,60,0.1)","#fb923c","rgba(251,146,60,0.28)"],["rgba(167,139,250,0.1)","#a78bfa","rgba(167,139,250,0.28)"],["rgba(244,114,182,0.1)","#f472b6","rgba(244,114,182,0.28)"]];
...
function ShareButtons({title,slug,compact}){
  ...
  if(compact)return(
    <div className="share-compact" onClick={e=>e.stopPropagation()}>
      <a href={liUrl} target="_blank" rel="noopener noreferrer" className="share-btn" title="LinkedIn"><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
      <a href={xUrl} target="_blank" rel="noopener noreferrer" className="share-btn" title="X"><svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
      <button className="share-btn" onClick={copyLink} title="Copy link">🔗</button>
    </div>
  );
  return(
    <div className="share-row" onClick={e=>e.stopPropagation()}>
      <a href={liUrl} target="_blank" rel="noopener noreferrer" className="share-btn-full"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg> LinkedIn</a>
      <a href={xUrl} target="_blank" rel="noopener noreferrer" className="share-btn-full"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> X</a>
      <button className="share-btn-full" onClick={copyLink}>🔗 Link</button>
    </div>
  );
}

function ImageUploader({images,setImages,t}){
  const ref=useRef();
  const add=async(e)=>{for(const f of Array.from(e.target.files)){if(!f.type.startsWith("image/"))continue;const b=await f2b(f);setImages(p=>[...p,{name:f.name,data:b}]);}e.target.value="";};
  return(<div><div className="field-label">{t.images}</div><div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>{images.map((img,i)=>(<div key={i} style={{position:"relative",width:56,height:56,borderRadius:6,overflow:"hidden",border:"1px solid var(--border2)"}}><img src={img.data} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/><button onClick={()=>setImages(p=>p.filter((_,j)=>j!==i))} style={{position:"absolute",top:1,right:1,background:"rgba(0,0,0,0.7)",color:"#fff",border:"none",borderRadius:"50%",width:16,height:16,fontSize:"0.55rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button></div>))}<button onClick={()=>ref.current?.click()} className="upload-btn">{t.addImage}</button></div><input ref={ref} type="file" accept="image/png,image/jpeg,image/gif,image/webp" multiple hidden onChange={add}/></div>);
}
function LinksEditor({links,setLinks,t}){
  return(<div><div className="field-label">{t.links}</div>{links.map((l,i)=>(<div key={i} style={{display:"flex",gap:6,marginBottom:6}}><input className="form-input" placeholder="Label" value={l.label} onChange={e=>{const v=e.target.value;setLinks(p=>p.map((x,j)=>j===i?{...x,label:v}:x));}} style={{width:110}}/><input className="form-input" placeholder="https://..." value={l.url} onChange={e=>{const v=e.target.value;setLinks(p=>p.map((x,j)=>j===i?{...x,url:v}:x));}} style={{flex:1}}/><button onClick={()=>setLinks(p=>p.filter((_,j)=>j!==i))} className="form-rm">✕</button></div>))}<button onClick={()=>setLinks(p=>[...p,{label:"",url:""}])} className="form-add-btn">+ {t.addLink}</button></div>);
}
...
function ContactFooter({t}){return(<div className="contact-footer"><div className="contact-inner"><div className="contact-label">{t.contactLabel}</div><h2 className="contact-title">{t.contactTitle}</h2><p className="contact-sub">{t.contactSub}</p><a href="https://linktr.ee/stevesenade" target="_blank" rel="noopener noreferrer" className="contact-link"><span>🔗</span> {t.contactCta}</a></div></div>);}
...
export default function App(){
  ...
  const[articles,setArticles]=useStore("2s_posts_v2",DEF_ARTICLES);
  const[creations,setCreations]=useStore("2s_crea_v2",DEF_CREATIONS);
  const[talks,setTalks]=useStore("2s_talks_v2",DEF_TALKS);
  ...
  const pinned=filtered.find(a=>a.pinned);const rest=filtered.filter(a=>!a.pinned);
  ...
  const togglePin=(id,e)=>{e.stopPropagation();setArticles(p=>p.map(a=>({...a,pinned:a.id===id?!a.pinned:false})));};
  ...
  const saveCreation=(data)=>{if(editItem){setCreations(p=>p.map(c=>c.id===editItem.id?{...c,...data}:c));notify(t.projectUpdated);}else{setCreations(p=>[{id:Date.now(),...data},...p]);notify(t.projectAdded);}setShowForm(null);setEditItem(null);};
  ...
  const saveTalk=(data)=>{if(editItem){setTalks(p=>p.map(x=>x.id===editItem.id?{...x,...data,date:x.date}:x));notify(t.talkUpdated);}else{setTalks(p=>[{id:Date.now(),date:new Date().toISOString().split("T")[0],...data},...p]);notify(t.talkAdded);}setShowForm(null);setEditItem(null);};
  const delTalk=(id)=>{setTalks(p=>p.filter(x=>x.id!==id));notify(t.talkDeleted);};

  const wc=draft.content.split(/\s+/).filter(Boolean).length;

  // Lang filter chips for any section
  ...
  return(
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;500;600;700;800&family=IBM+Plex+Serif:ital,wght@0,300;0,400;0,500;1,300;1,400&family=IBM+Plex+Mono:wght@400;500&display=swap');
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        :root{--bg:#07090f;--bg2:#0c1018;--bg3:#111826;--border:rgba(255,255,255,0.055);--border2:rgba(255,255,255,0.1);--text:#dde4f0;--text2:#7d8fa6;--text3:#404d5e;--accent:#4f9cf9;--accent2:#82baff;--accent-dim:rgba(79,156,249,0.1);--accent-glow:rgba(79,156,249,0.22);--green:#34d399;--orange:#fb923c;--red:#f87171;--fd:'Syne',sans-serif;--fb:'IBM Plex Serif',Georgia,serif;--fm:'IBM Plex Mono',monospace}
        ...
        .nav{position:sticky;top:0;z-index:200;background:rgba(7,9,15,0.92);backdrop-filter:blur(20px);border-bottom:1px solid transparent;transition:border-color 0.3s}.nav.stuck{border-bottom-color:var(--border)}.nav-inner{max-width:1100px;margin:0 auto;height:60px;padding:0 2rem;display:flex;align-items:center;justify-content:space-between;gap:0.75rem}
        .logo{font-family:var(--fd);font-size:1.1rem;font-weight:800;color:var(--text);cursor:pointer;display:flex;align-items:center;gap:10px;transition:opacity 0.2s;flex-shrink:0;background:none;border:none}.logo:hover{opacity:0.75}.logo-mark{width:30px;height:30px;background:var(--accent);border-radius:7px;display:flex;align-items:center;justify-content:center;font-size:0.65rem;font-weight:800;color:#fff;box-shadow:0 0 14px var(--accent-glow);flex-shrink:0}
        .nav-links{display:flex;align-items:center;gap:0.35rem;font-family:var(--fd);font-size:0.82rem;font-weight:600}.nav-dot{color:var(--text3);padding:0 0.2rem;user-select:none}.nav-link{color:var(--text2);cursor:pointer;padding:0.3rem 0.55rem;border-radius:6px;transition:all 0.15s;border:none;background:none;font:inherit;font-weight:600}.nav-link:hover{color:var(--text);background:var(--bg3)}.nav-link.active{color:var(--accent2);background:var(--accent-dim)}
        .nav-right{display:flex;align-items:center;gap:0.6rem;flex-shrink:0}
        .lang-toggle{font-family:var(--fd);font-size:0.68rem;font-weight:700;background:var(--bg3);color:var(--text2);border:1px solid var(--border2);border-radius:6px;padding:0.3rem 0.55rem;cursor:pointer;transition:all 0.15s;display:flex;align-items:center;gap:4px}.lang-toggle:hover{color:var(--text);border-color:rgba(255,255,255,0.2)}
        .btn-new{font-family:var(--fd);font-size:0.75rem;font-weight:700;background:var(--accent);color:#fff;border:none;border-radius:8px;padding:0.45rem 1rem;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all 0.18s;box-shadow:0 0 14px var(--accent-glow);white-space:nowrap}.btn-new:hover{background:var(--accent2);transform:translateY(-1px)}
        ...
        .post-list{display:flex;flex-direction:column}.post-item{padding:1.25rem 0;border-bottom:1px solid var(--border);cursor:pointer;transition:all 0.15s;display:flex;align-items:center;gap:0.75rem}.post-item:first-child{padding-top:0}.post-item:hover .post-title{color:var(--accent2)}.post-date{font-family:var(--fd);font-size:0.7rem;color:var(--text3);min-width:80px;flex-shrink:0}.post-title{font-family:var(--fd);font-size:0.95rem;font-weight:700;color:var(--text);line-height:1.35;transition:color 0.15s;flex:1}.post-pin{font-size:0.72rem;opacity:0.4;cursor:pointer;flex-shrink:0;transition:opacity 0.15s;background:none;border:none;padding:2px}.post-pin:hover{opacity:1}
        .featured-post{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:1.5rem;margin-bottom:1.75rem;cursor:pointer;transition:all 0.2s;position:relative;overflow:hidden}.featured-post::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--accent),var(--accent2),transparent)}.featured-post:hover{border-color:rgba(79,156,249,0.25);transform:translateY(-1px);box-shadow:0 12px 40px rgba(0,0,0,0.35)}.featured-label{font-family:var(--fd);font-size:0.6rem;letter-spacing:0.14em;text-transform:uppercase;color:var(--accent);margin-bottom:0.75rem}.featured-title{font-family:var(--fd);font-size:1.2rem;font-weight:800;line-height:1.25;margin-bottom:0.5rem}.featured-excerpt{font-family:var(--fd);font-size:0.82rem;color:var(--text2);line-height:1.6}.featured-meta{font-family:var(--fd);font-size:0.68rem;color:var(--text3);margin-top:1rem;padding-top:0.75rem;border-top:1px solid var(--border);display:flex;justify-content:space-between}
        .creation-card{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:1.5rem;margin-bottom:0.75rem;transition:all 0.2s;position:relative}.creation-card:hover{border-color:rgba(79,156,249,0.2);transform:translateX(3px)}.creation-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:0.6rem;gap:0.5rem}.creation-title{font-family:var(--fd);font-size:1rem;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px}.creation-status{font-family:var(--fd);font-size:0.58rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;padding:0.2rem 0.55rem;border-radius:4px;flex-shrink:0}.creation-desc{font-family:var(--fd);font-size:0.82rem;color:var(--text2);line-height:1.6;margin-bottom:0.85rem}.creation-tech{display:flex;gap:0.35rem;flex-wrap:wrap;margin-bottom:0.5rem}.tech-tag{font-family:var(--fm);font-size:0.62rem;padding:0.15rem 0.5rem;border-radius:3px;background:var(--bg3);color:var(--text3);border:1px solid var(--border)}.creation-links{display:flex;gap:0.5rem;flex-wrap:wrap}.creation-link{font-family:var(--fd);font-size:0.7rem;font-weight:600;color:var(--accent2);text-decoration:none;display:flex;align-items:center;gap:3px}.creation-link:hover{opacity:0.7}.creation-media{display:flex;gap:6px;margin-top:0.75rem;flex-wrap:wrap}.creation-media img{width:56px;height:56px;object-fit:cover;border-radius:6px;border:1px solid var(--border2)}.creation-actions{position:absolute;top:10px;right:10px;display:flex;gap:4px;opacity:0;transition:opacity 0.15s}.creation-card:hover .creation-actions{opacity:1}
        .talk-card{background:var(--bg2);border:1px solid var(--border);border-radius:10px;padding:1.5rem;margin-bottom:0.75rem;transition:all 0.2s;position:relative}.talk-card:hover{border-color:rgba(79,156,249,0.2);transform:translateX(3px)}.talk-top{display:flex;align-items:center;gap:0.75rem;margin-bottom:0.6rem}.talk-icon{font-size:1.4rem;width:40px;height:40px;display:flex;align-items:center;justify-content:center;background:var(--bg3);border-radius:8px;flex-shrink:0}.talk-type{font-family:var(--fd);font-size:0.6rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--accent)}.talk-title{font-family:var(--fd);font-size:1rem;font-weight:700;color:var(--text);line-height:1.3;display:flex;align-items:center;gap:6px}.talk-show{font-family:var(--fd);font-size:0.72rem;color:var(--text3);margin-top:1px}.talk-desc{font-family:var(--fd);font-size:0.82rem;color:var(--text2);line-height:1.6;margin-bottom:0.75rem}.talk-footer{display:flex;gap:1rem;font-family:var(--fd);font-size:0.68rem;color:var(--text3);align-items:center}.talk-play{font-family:var(--fd);font-size:0.7rem;font-weight:700;color:var(--accent2);cursor:pointer;background:none;border:none;display:flex;align-items:center;gap:4px;margin-left:auto}.talk-play:hover{opacity:0.7}.talk-actions{position:absolute;top:10px;right:10px;display:flex;gap:4px;opacity:0;transition:opacity 0.15s}.talk-card:hover .talk-actions{opacity:1}
        ...
        .item-actions-btn{font-family:var(--fd);font-size:0.6rem;font-weight:600;background:var(--bg3);color:var(--text2);border:1px solid var(--border2);border-radius:4px;padding:0.2rem 0.45rem;cursor:pointer;transition:all 0.12s}.item-actions-btn:hover{color:var(--text)}.item-actions-btn.del{color:var(--red);border-color:rgba(248,113,113,0.2)}
        ...
        .related-section{margin-top:3.5rem;padding-top:2rem;border-top:1px solid var(--border)}.related-label{font-family:var(--fd);font-size:0.6rem;letter-spacing:0.14em;text-transform:uppercase;color:var(--text3);margin-bottom:1rem}.related-card{display:flex;gap:1rem;padding:0.9rem;background:var(--bg2);border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:all 0.15s;margin-bottom:0.5rem}.related-card:hover{border-color:rgba(79,156,249,0.25);transform:translateX(4px)}
        .md{font-family:var(--fb);font-size:1.05rem;line-height:1.85;color:#b4c4d8}.md h1{font-family:var(--fd);font-size:1.8rem;font-weight:800;color:var(--text);margin:2.5rem 0 1rem;letter-spacing:-0.025em;line-height:1.2}.md h2{font-family:var(--fd);font-size:1.3rem;font-weight:700;color:var(--text);margin:2.25rem 0 0.75rem;padding-left:1rem;border-left:2px solid var(--accent)}.md h3{font-family:var(--fd);font-size:1.05rem;font-weight:700;color:var(--text);margin:1.75rem 0 0.5rem}.md p{margin-bottom:1.25rem}.md strong{color:var(--text);font-weight:600}.md em{color:var(--text2);font-style:italic}.md blockquote{margin:1.75rem 0;padding:1rem 1.5rem;background:var(--bg2);border-left:3px solid var(--accent);border-radius:0 8px 8px 0;color:var(--text2);font-style:italic}.md ul,.md ol{padding-left:1.5rem;margin-bottom:1.25rem}.md li{margin-bottom:0.4rem}.md li::marker{color:var(--accent)}.md code{font-family:var(--fm);background:var(--bg3);color:#93c5fd;padding:0.12em 0.4em;border-radius:4px;font-size:0.875em;border:1px solid var(--border2)}.md pre{background:#040609;border:1px solid var(--border2);border-radius:10px;padding:1.5rem;overflow-x:auto;margin:1.75rem 0;position:relative}.md pre code{background:none;border:none;color:#7dd3fc;padding:0;font-size:0.875rem;line-height:1.7}.md hr{border:none;margin:2.5rem 0;height:1px;background:var(--border)}.md img{max-width:100%;border-radius:8px;margin:1.5rem 0}
        ...
        .ed-body{flex:1;display:flex;overflow:hidden}.wpane,.ppane{flex:1;display:flex;flex-direction:column;overflow:hidden}.pane-lbl{background:var(--bg2);border-bottom:1px solid var(--border);padding:0.25rem 1.25rem;font-family:var(--fd);font-size:0.58rem;letter-spacing:0.14em;text-transform:uppercase;color:var(--text3)}.wpane{border-right:1px solid var(--border)}textarea{flex:1;background:var(--bg);color:#8da0b8;border:none;outline:none;resize:none;font-family:var(--fm);font-size:0.875rem;line-height:1.75;padding:1.5rem;tab-size:2}.pscroll{flex:1;overflow-y:auto;padding:2rem 2.5rem}
        ...
        .btn-pub{font-family:var(--fd);font-size:0.78rem;font-weight:700;background:var(--accent);color:#fff;border:none;border-radius:8px;padding:0.45rem 1.3rem;cursor:pointer;transition:all 0.18s;box-shadow:0 0 12px var(--accent-glow)}.btn-pub:hover{background:var(--accent2);transform:translateY(-1px)}
        .mo{position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:1000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.18s;padding:1rem;overflow-y:auto}.mo-box{background:var(--bg2);border:1px solid var(--border2);border-radius:14px;padding:2rem;max-width:520px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.5);max-height:90vh;overflow-y:auto}.mo-title{font-family:var(--fd);font-size:1.05rem;font-weight:700;text-align:center;margin-bottom:0.75rem}.mo-desc{font-family:var(--fd);font-size:0.8rem;color:var(--text2);text-align:center;margin-bottom:1.5rem}.mo-cancel{flex:1;font-family:var(--fd);font-weight:600;font-size:0.8rem;background:var(--bg3);color:var(--text2);border:1px solid var(--border2);border-radius:8px;padding:0.6rem;cursor:pointer}.mo-cancel:hover{color:var(--text)}.mo-delete{flex:1;font-family:var(--fd);font-weight:700;font-size:0.8rem;background:rgba(248,113,113,0.15);color:var(--red);border:1px solid rgba(248,113,113,0.3);border-radius:8px;padding:0.6rem;cursor:pointer}.mo-save{flex:1;font-family:var(--fd);font-weight:700;font-size:0.8rem;background:var(--accent);color:#fff;border:none;border-radius:8px;padding:0.6rem;cursor:pointer;box-shadow:0 0 12px var(--accent-glow)}.mo-save:hover{background:var(--accent2)}@keyframes fadeIn{from{opacity:0}to{opacity:1}}
        .form-grid{display:flex;flex-direction:column;gap:0.85rem}.field-label{font-family:var(--fd);font-size:0.68rem;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:var(--text3);margin-bottom:0.3rem}.form-input{width:100%;background:var(--bg);border:1px solid var(--border2);border-radius:6px;padding:0.5rem 0.75rem;color:var(--text);font-family:var(--fd);font-size:0.82rem;outline:none;transition:border-color 0.15s}.form-input:focus{border-color:rgba(79,156,249,0.5)}.form-input::placeholder{color:var(--text3)}select.form-input{appearance:none;cursor:pointer}.form-ta{resize:vertical;min-height:60px;font-family:var(--fd)}.form-rm{background:none;border:none;color:var(--text3);cursor:pointer;font-size:0.8rem;padding:0.3rem}.form-rm:hover{color:var(--red)}.form-add-btn{font-family:var(--fd);font-size:0.72rem;font-weight:600;color:var(--accent2);background:none;border:1px dashed var(--border2);border-radius:6px;padding:0.35rem;cursor:pointer;width:100%}.form-add-btn:hover{border-color:var(--accent);background:var(--accent-dim)}
      ...
      {section==="posts"&&view==="list"&&(<><div className="page-wrap">
        <div className="site-intro"><div className="intro-eyebrow">{t.heroEye}</div><h1 className="intro-title">{t.heroTitle[0]}<em>{t.heroTitle[1]}</em></h1><p className="intro-sub">{t.heroSub}</p></div>
        <div className="filter-row">
          <div className="filters"><button className={`ftag ${!filterTag?"on":""}`} onClick={()=>setFilterTag(null)}>{t.all} <span className="ftag-count">{articles.length}</span></button>{allTags.map(x=><button key={x} className={`ftag ${filterTag===x?"on":""}`} onClick={()=>setFilterTag(filterTag===x?null:x)}>{x} <span className="ftag-count">{tagCounts[x]}</span></button>)}</div>
          <LangFilters/>
        </div>
        {pinned&&!filterTag&&!filterLang&&(<div className="featured-post" onClick={()=>{setCurrent(pinned);setView("article");}}><div className="featured-label">★ {t.featured}</div><div style={{display:"flex",gap:"0.4rem",marginBottom:"0.6rem",flexWrap:"wrap"}}>{pinned.tags.map((x,i)=><span key={x} className="chip" style={cs(i)}>{x}</span>)}<LangChip lang={pinned.lang}/></div><div className="featured-title">{pinned.title}</div><div className="featured-excerpt">{exc(pinned.content)}…</div><div className="featured-meta"><span>{fDate(pinned.date,lang)} · {rt(pinned.content)} {t.readMin}</span><span style={{color:"var(--accent2)",fontWeight:700}}>{t.read} →</span></div></div>)}
        {filtered.length===0?<div className="empty"><h3>{t.noResults}</h3></div>:(<div className="post-list">{rest.map(a=>(<div key={a.id} className="post-item" onClick={()=>{setCurrent(a);setView("article");}}><span className="post-date">{fDate(a.date,lang)}</span><LangChip lang={a.lang}/><span className="post-title">{a.title}</span><button className="post-pin" onClick={e=>togglePin(a.id,e)}>📌</button></div>))}</div>)}
      ...
      {view==="editor"&&(<div className="ed-wrap">
        <div className="ed-bar">
          <input className="ed-title" placeholder={t.title+"..."} value={draft.title} onChange={e=>setDraft(d=>({...d,title:e.target.value}))}/>
          <div className="ed-sep"/>
          <input className="ed-tags" placeholder="Tags: Sales, AI..." value={draft.tags} onChange={e=>setDraft(d=>({...d,tags:e.target.value}))}/>
          <div className="ed-sep"/>
          <select className="ed-lang" value={draft.lang} onChange={e=>setDraft(d=>({...d,lang:e.target.value}))}><option value="FR">🇫🇷 FR</option><option value="EN">🇬🇧 EN</option></select>
          <div style={{marginLeft:"auto"}}><div className="tabs">{[[" write",t.write],["split",t.split],["preview",t.previewTab]].map(([v,l])=><button key={v} className={`tbtn ${tab===v.trim()?"on":""}`} onClick={()=>setTab(v.trim())}>{l}</button>)}</div></div>
        </div>
        ...
        <div className="ed-body">
          {tab!=="preview"&&<div className="wpane"><div className="pane-lbl">{t.markdown}</div><textarea ref={taRef} value={draft.content} onChange={e=>setDraft(d=>({...d,content:e.target.value}))} spellCheck={false}/></div>}
          {tab!=="write"&&<div className="ppane"><div className="pane-lbl">{t.preview}</div><div className="pscroll"><div className="md" dangerouslySetInnerHTML={{__html:parseMd(draft.content||"_..._")}}/>{images.length>0&&<div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:"1rem"}}>{images.map((img,i)=><img key={i} src={img.data} alt="" style={{maxWidth:200,borderRadius:8,border:"1px solid var(--border2)"}}/>)}</div>}</div></div>}
        </div>
